            ...
            <table class="table table-hover" id="tabla">
                <thead class="thead-light">
                    <tr>
                        ...
                        <th style="width: 50%">Atendida</th>
                    </tr>
                </thead>
                <tbody>
                    <tr   v-for="(n, index) in notificaciones" :key="index" class="cursor-pointer" @click="irNotificacion(n) " :class="n.atendida ? 'atendido' : 'no-atendido'">
...
<script type="text/javascript">
    ...
    var app = new Vue({
        ...
        methods: {
            ...
            toggleAtendida(n) {                
                const nuevoValor = n.atendida ? 1 : 0;

                axios.post(`api/notificaciones/${n.id}/atendida`, {
                        atendida: nuevoValor
                    })
                    .then(response => {                        
                        n.atendida = response.data.atendida == 1 ? true : false;                        
                    })
                    .catch(error => {
                        console.error("Error al actualizar:", error);                        
                        n.atendida = !n.atendida;
                    });
            }
        },
